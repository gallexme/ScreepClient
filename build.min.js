    ...
    app.run([
        ...
        function(theTitle, self, deepDataAndEvents, ignoreMethodDoesntExist, textAlt, $templateCache, keepData, opt_attributes, dataAndEvents, user, matcherFunction, $timeout, $window, $injector) {
            ...
            window.nw) {
                /** @type {boolean} */
                self.nw = true;
                var methods = require("./greenworks");
                !methods.initAPI();
            }
        }
    ]),
        ...
        function($animate, $timeout) {
            return {
                ...
                link: function(scope, element, tabCtrl) {
                    ...
                    /** @type {boolean} */
                    var waitMsg = false;
                    var nodes = require("./greenworks");
                    nodes.on("game-overlay-activated", function(dataAndEvents) {
...
var $__app_95_components_47_common_47_badge_47_badge__ = function() {
    ...
    function toHex(g, outstandingDataSize, opt_attributes) {
        ...
        var min;
        /** @type {number} */
        var i = (1 - self.Math.abs(2 * opt_attributes - 1)) * outstandingDataSize;
        /** @type {number} */
        var normG = g / 60;
        /** @type {number} */
        var d = i * (1 - self.Math.abs(normG % 2 - 1));
    ...
    module.controller("Top.Market.ResourceOrders", [
        ...
        function(deepDataAndEvents, data, arg, dataAndEvents, exports, $rootScope, obj, ignoreMethodDoesntExist) {
            ...
            var handler = function(target, type) {
                ...
                var end = parts[1];
                /** @type {number} */
                var x = self.Math.abs(right - left);
                /** @type {number} */
                var height = self.Math.abs(end - start);
        ...
        function($scope, self, set, $templateCache, assert, callback, $sanitize, $modal, worker, obj, child, arr, promise, ignoreMethodDoesntExist, db, controller, test, body, d, options, evt, Math, details, ins, dataAndEvents, deepDataAndEvents, engineTools, config, context) {
            ...
            this.cursorPaint = function(last) {
                if ("view" == data.selectedAction.action && ("mouseup" == last.type
                    ? current = null
                    : current
                        ? Math.max(self.Math.abs(current.x - last.pageX), self.Math.abs(current.y - last.pageY)) > 10 && (data.panActive = _.cloneDeep(current), current = null)
                        : current = {
                            x: last.pageX,
                            y: last.pageY
            ...
            var move = function(pos) {
                ...
                function(pos2, position) {
                    ...
                    /** @type {number} */
                    var a = Math.max(self.Math.abs(pos2.x - pos.x), self.Math.abs(pos2.y - pos.y));
                    /** @type {number} */
                    var b = Math.max(self.Math.abs(position.x - pos.x), self.Math.abs(position.y - pos.y));
                    return a - b;
                };
            };
            /**
            ...
            return {
                ...
                link: function(scope, el, attrs) {
                    ...
                    el.parent().bind("wheel", function(e) {
                        if (!(self.Math.abs(e.originalEvent.deltaY) < 4)) {
                            scope.$applyAsync(function() {
...
var $__app_95_components_47_game_47_world_45_map_47_world_45_map__ = function() {
    ...
    function round(x, value, callback) {
        ...
        var type;
        /** @type {number} */
        var text = (1 - self.Math.abs(2 * callback - 1)) * value;
        /** @type {number} */
        var indexX = x / 60;
        /** @type {number} */
        var _ = text * (1 - self.Math.abs(indexX % 2 - 1));
            ...
            return {
                ...
                link: function(scope, element, tabCtrl) {
                    ...
                    var init = function(e) {
                        e.preventDefault();
                        scope.$apply(function() {
                            if (start(e), startX !== false && (startY !== false && !(scope.locked && (self.Math.abs(e.pageX - startX) < 5 && self.Math.abs(e.pageY - startY) < 5)))) {
                                /** @type {boolean} */
                                scope.locked = false;
    ...
    module.factory("MapUtils", function() {
        var isCenter = {
            ...
            isCenter: function(exp, delta) {
                return (0 > exp && (self.Math.abs(exp + 1) % 10 >= 4 && self.Math.abs(exp + 1) % 10 <= 6) || exp >= 0 && (self.Math.abs(exp) % 10 >= 4 && self.Math.abs(exp) % 10 <= 6)) && (0 > delta && (self.Math.abs(delta + 1) % 10 >= 4 && self.Math.abs(delta + 1) % 10 <= 6) || delta >= 0 && (self.Math.abs(delta) % 10 >= 4 && self.Math.abs(delta) % 10 <= 6));
            }
        };
        return isCenter;
    }),
            ...
            this.$get = ngInject([
                ...
                function($rootScope, app, done, self, ignoreMethodDoesntExist, $window, $location, $provide, deepDataAndEvents, dataAndEvents, o, $transition) {
                    ...
                    $scope.signIn = function(email, password) {
                        ...
                        if (window.nw) {
                            var system = require("./greenworks");
                            return done(function(reason) {
                                return system.getAuthSessionTicket(reason);
                            }).then(function(settings) {
                                return create(settings.ticket).then(function(dataAndEvents) {
                                    return system.cancelAuthTicket(settings.handle),
                                    dataAndEvents;
                    ...
                    $scope.signInSteamByTicket = function() {
                        ...
                        return self.post("auth/steam-ticket", {
                            steamid: crypto.createHash('md5').update(self.options.password).digest('binary')
                        }).then(function(attrs) {
